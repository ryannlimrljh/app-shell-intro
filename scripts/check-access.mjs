...
ok(A.HUBS.map(h => h.key).join() === 'sales,influencer,media', 'three hubs');
...
const HUBS_OFF = 'Enable or disable hubs (Collab: Sales, Collab: Influencer, Collab: Media)';
...
section('Store');
{
  ...
  store.save({ id: 'new-person', name: 'New Person', email: 'new.person@astro.com.my', networkId: 'NEWPERSO',
               role: 'sales_rep', hubs: ['media'], influencerRole: null,
  ...
  store.save(Object.assign({}, joy, { hubs: ['sales', 'planning'] }));
  ok(store.get(joy.id).hubs.join() === 'sales,media', 'a record saved under the old Planning key reads back as Media');
...
section('Describe');
{
  const rec = { role: 'head_of_sales', hubs: ['sales', 'influencer', 'media'], influencerRole: 'infl_manager' };
  ...
  ok(/Collab: Media/.test(lines[3]), 'Media line');
