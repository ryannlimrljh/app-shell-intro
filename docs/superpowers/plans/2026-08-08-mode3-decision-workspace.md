...
### Holistic review round — nine findings outside the five task regions

52. **Critical — Watch rendered above Now, inverting the mode's own thesis.** Task 2 appended the Watch section to the then-current markup; Task 3 appended the decision list after it. Both diffs were correct in isolation, so twelve task-scoped review passes never saw the two together. Shipped order was Posture → Health → **Watch** → **Now** → decisions — under a heading reading "Three decisions this week," a reader met four non-actionable cards before a single decision. Fixed with a pure markup move (no CSS, no JS): the Watch `.m3-srule` + `.m3-watch` grid now sit immediately after `.decs` closes, between the decision list and `.m3-asks`, restoring spec order Posture → Health → Now → Also open → Watch → Evidence → Log. `reflowDecisions` only touches children of `#m3Decs`, so it was structurally unaffected — confirmed by the widget-count gate (15/12/11, zero leaks) both before and after the move, and by measuring "Now" now at ~1319px and "Watch" at ~3251px in a 1440px viewport.

53. **Important — the section heading didn't know about the horizon filter.** `titles.decide[0]` was a fixed string ("Three decisions this week") written once by `setMode`; the horizon radiogroup re-tiers the decision list without touching it, so switching to FY outlook left "Three decisions this week" standing over one promoted card. Added a `decideTitle` map (`week`/`quarter`/`fy` → the count-correct string) and a shared `updateDecideTitle()` read by both `setMode`'s decide branch and the horizon click handler, so re-entering Mode 3 at a non-week horizon is also correct — verified explicitly (switch to quarter, leave to Mode 1, return to Mode 3: heading still reads "Three decisions this quarter"). Heading count matched the DOM's actual promoted-card count at all three horizons (3/3/1).

54. **Important — `mQuad`'s Mode 3 inset and Mode 2's own `cQuad` data disagreed on Aisyah Rahman's cover ratio**, and Mode 2's own prose (`2.4× cover`) had already sided against its own chart (`y:2.5`). The Mode 3 inset (built correctly in Task 4) was never the problem; Mode 2's `cQuad` `reps` array was. One-character fix, `y:2.5`→`y:2.4` in the `reps` array — confirmed **not** to touch the three prose sites, which already read `2.4×`. Re-verified via the built drawer HTML (`Aisyah Rahman · 118% · 2.4×`) and via Mode 2's own `reps` array now reading `y:2.4`; the "2.5×" a naive grep still finds nearby is the Y-axis gridline label (a fixed `[0.5,1.0,1.5,2.0,2.5]` tick array, unrelated to any rep's data) — false-alarmed on this once during verification before tracing it to the axis-tick generator two lines away.

55. **Important — the NRR tile's "derived from the growth bridge" line quoted the wrong lapsed figure.** The tile read "lapsed RM 4.4M YTD"; the bridge it claims to derive from uses lapsed = **8.6** (the four *named* lapses in the churn panel sum to 4.4, a different and smaller number). A reader auditing the Derived tag with the tile's own printed inputs would compute 100.3%, not 97.3%, in the one file whose thesis is that provenance tags are load-bearing. Changed the tile's second line to "lapsed RM 8.6M in the bridge"; left the D6 card's own "RM 2.1M of the RM 4.4M lapsed" prose alone, since that one is correctly talking about the four named lapses, not the bridge total.

56. **Important — twenty-one Approve/Ask for options/Defer with reason buttons were inert**, the most decision-shaped control on every card doing nothing in a mode called "the decision workspace." This was a judgement call the coordinator made explicitly (the spec's four listed interactions did not include these), and is flagged as such rather than treated as a plan defect. Wired through the existing machinery rather than a parallel path: `logRow` gained a `(dec,label,cls)` signature so a log entry's visible text and colour class no longer have to be a chip status; `cycleStatus` factored into a shared `setStatus(chip,status)` primitive; **Approve** → `setStatus(chip,'Scheduled')`, **Ask for options** → `setStatus(chip,'Awaiting client')`, **Defer with reason** → `logRow(dec,'Deferred','o-wait')` with no change to `chip.dataset.status`. The five-word status vocabulary (`STATUSES`) was not extended — "Deferred" is a log-only label, never assigned to a chip. All three verified with real pointer input (`computer` clicks, landing confirmed via a capture-phase listener) on three different cards: Approve turned D1's chip "Scheduled" and wrote a log row; Ask for options turned D2's chip "Awaiting client"; Defer with reason left D3's chip at "Not started" while writing a "Deferred" log row and updating its live span to "logged · deferred."

57. **Important — every generated log row attributed the change to Bryan Wong**, including for cards Bryan Wong doesn't own (Aisyah Rahman owns D4 and D6, Daniel Tan owns D3). Added `data-owner` to each `<section class="dec">`, read from that card's own already-printed Owner field, and `logRow` now reads `dec.dataset.owner` instead of a hardcoded string (falling back to 'Bryan Wong' only if the attribute is ever missing). Verified by clicking Ask for options on D2 (owner "Leadership") and Defer on D3 (owner "Daniel Tan") — both log rows carry the correct name. The literal string `'today'` was also replaced with a fixed in-universe `TODAY='8 Aug'` constant (one day after the masthead's own "REVIEWED 7 AUG 2026," consistent with the seed rows' 5/1 Aug and 28/21 Jul dates) rather than the real device clock, which would print whatever date the page happens to be opened on and silently disagree with the fictional dataset around it.

58. **Important — Mode 2's own banner miscounted its own panels, and Mode 3's now-correct "nine analytical panels" made the discrepancy discoverable two clicks away.** Counted from the live DOM by provenance dot combination (not eyeballed): **9** chart panels total; **6** carry both a confirmed/computed dot and a pending dot (`cFan`, `cQuad`, `cFill`, `cBubble`, `cChurn`, `cShare`); **2** carry only a pending dot (`cFunnel`, `cConc`); **1** carries only a computed dot with no pending (`cBridge`). No reading of the DOM produces 11, 8, or 5. Fixed both the banner (`copy.next`, "8 of the 11 panels mix both … The five pending panels are plausible") and — beyond the coordinator's literal instruction, but the same defect — the short provenance-legend caption a few widgets earlier (`8 of 11 panels mix data you have with a source still to land.`), since leaving the second instance uncorrected would have had Mode 2 visibly disagree with itself one scroll below its own fixed banner. Both now read "6 of the 9…two are pending outright" / "6 of 9 panels…" respectively. Mode 3's Evidence heading ("nine analytical panels") was already correct and untouched.

59. **Minors, all taken.** `.m3-hz-lbl` repointed from `--art-faint` (3.44:1 light / 3.30:1 dark, 10px) to `--art-muted` — measured 6.90:1/6.61:1 light and 7.08:1/7.65:1 dark across both instances of the class (the horizon label and the reused "Questions this board can now answer" label). The file-wide `--art-faint` token itself was left untouched, per instruction. "Now"'s subtitle reworded from "nobody below can unlock these" (false once D5/D6, both "Escalation: None," sit under Now at the quarter horizon) to "Decisions in the selected horizon, at full weight." The Share-of-market Watch card's "at FY horizon this becomes a decision" claim is now true: `[data-horizon="fy"] .m3-watch .m3-wcard:nth-child(3)` dims to `opacity:.55` and reveals a "Now a decision above — D7" note, both inert at week/quarter — screenshot-confirmed both states. D6's compact summary now reads "RM 2.1M · Aisyah Rahman · by the Q3 reset window · decided 5 Aug," matching the other six cards' pattern of stating a deadline. The forecast tile now reads "RM 222.9M **weighted**" in a smaller trailing label, disambiguating it from Mode 1's RM 228.1M and RM 222.3M on other bases two clicks away. `setMode`'s `now` branch now consults `root.dataset.view` before falling back to the generic leadership subtitle — verified: pick Sales manager, switch to Mode 2 and back to Mode 1, subtitle still reads "My team, my book, my targets" instead of reverting.

60. **Deferred, not done, per explicit instruction:** the evidence deep link still has no return path from Mode 2 back to Mode 3 — landing ~2,000px from where you left if you use the mode row instead of the (nonexistent) back control. Real gap, recorded as follow-up, needs design thought rather than a patch.
